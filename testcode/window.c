...
static void HBB_objLoader(void);
...
static int _windowWidth = 800, _windowHeight = 600,_landscape_w = 303, _landscape_h = 613;
...
//////////////// OBJET LOADING //////////////////
#define OBJMAX 15
#define OBJNB 11// Pour ajouter un objet,augumenter la valeur +1
...
static char *OBJFilePath[]={"Objet3D/M4A1/M4A1.obj"
			,"Objet3D/F-15/F-15C_Eagle.obj"
			,"Objet3D/AircraftCarrier/AircraftCarrier.obj"
			,"Objet3D//MK83Bomb/Files/mk83.obj"			
			,"Objet3D/Flak37/Flak37.obj"
			,"Objet3D/pickaxe/pickaxe.obj"
			,"Objet3D/shovel/schaufel.obj"
...
static char *TEXFilePath[]={"Texture/skybox1.png"
 			   ,"Texture/skyb.png"
 			   ,"Texture/skyboxTop.png"
 			   ,"Texture/wall.jpg"
 			   ,"Texture/top.jpg"
 			   ,"Texture/right.jpg"
			   ,"Texture/a.png"
			   ,"Texture/left.jpg"
			   ,"Texture/TOP3.jpg"
			   ,"Texture/camouflage.jpg"
			   ,"Texture/camouflage2.jpg"
			   ,"Texture/metalBlack.jpg"
			   ,"Texture/camouflage3.jpeg"
			   ,"Texture/mk82.png"
			   ,"Texture/flame.jpg"
};
...
for(i=0;i<NB_missile;i++)
	...
	missileTab[i].missileY += (h-missileTab[i].mouseY)/(GLfloat)h; 
...
for(i=0;i<NBAVIONAI;i++){
	avionAI[i].x=rand()%(WORLDSCALE/5)-(WORLDSCALE/5);
...
int i,rapide=2000; 
GLfloat cadreTaille=WORLDSCALE-3000;
...
GLfloat theta,thetaHauter;
int bulletNB;
bullet_st bullet[30000];	
};
#define NBTURRET 2400
...
defenseAI[i].x=4050+rand()%30-30;
defenseAI[i].y=150+rand()%20;
defenseAI[i].z=-2600+rand()%30-30;
}
for(i=2200;i<2400;i++){
defenseAI[i].x=1050+rand()%40-40;
defenseAI[i].y=150+rand()%20;
defenseAI[i].z=-2500+rand()%40-40;
}
...
static GLuint _explosionVao;
static GLuint _explosionBuffer[2];
static GLuint _explosionTex;
static int * im=NULL;
Uint32 * pixels=NULL;

int _latitudes=30;
int _longitudes=30;
static void HBB_initExplosion(void){
...
int TEXH=1000;
 ...
 if(!_explosionVao)
    glGenVertexArrays(1, &_explosionVao);
  glBindVertexArray(_explosionVao);
  glEnableVertexAttribArray(0);
  glEnableVertexAttribArray(1);
  if(!_explosionBuffer[0])
    glGenBuffers(2,_explosionBuffer);
  /* sphère où tous les sommets ne sont présent qu'une fois */
  glBindBuffer(GL_ARRAY_BUFFER, _explosionBuffer[0]);
  ...
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _explosionBuffer[1]);
...
static void initGL(void) {
  ...
  _pId2 = gl4duCreateProgram("<vs>shaders/basic2.vs", "<gs>shaders/basic2.gs", "<fs>shaders/basic2.fs", NULL);
...
static int ANM_avionNB=5;
static avion_t ANM_avion[5];

static void initAnimation1(void){
int i;

for(i=0;i<ANM_avionNB;i++){
ANM_avion[i].x=0;
ANM_avion[i].y=370;
ANM_avion[i].z=10;
ANM_avion[i].theta =_cam.theta;// -6.00 ~ 6.00
ANM_avion[i].thetaHauter=0;// 0.00 ~ 0.75
ANM_avion[i].force=0;//0 ~ 1
ANM_avion[i].centre=0; //-1.5 ~ 1.5 z
ANM_avion[i].centre2=0; //-1.5 ~ 1.5 y
ANM_avion[i].centre3=0; //0.0 ~ 1.0 x
}

}

static void drawAnimation1(){
int i;
...
for(i=0;i<ANM_avionNB;i++){
	gl4duPushMatrix();{	
  	...
  	//gl4duLoadIdentityf();
	gl4duTranslatef(ANM_avion[i].x,ANM_avion[i].y,ANM_avion[i].z-300); 
...
avion.centre2=(((_windowHeight/2)-ym)/(double) _windowHeight)*3.0*0.3;
avion.centre3=-(((_windowWidth/2)-xm)/(double)_windowWidth)*3.0*0.3;
...
if(_weaponMode==MODE_PICKAXE)
_weaponMode=MODE_SHOVEL;
else
_weaponMode= MODE_PICKAXE;
break;
  case 'm':// weapon mode gun 
if(_weaponMode==MODE_GUN)
_weaponMode=MODE_FLAK37;
else 
_weaponMode = MODE_GUN;
break;
...
SDL_PumpEvents();
SDL_GetMouseState(&xm, &ym);
missileTab[NB_missile].mouseY=ym;
...
static void HBB_drawSkybox(void) {
  ...
  glBindTexture(GL_TEXTURE_2D,_tabTexId[TEX_SKYBOX2]);
...
int xm, ym;
	SDL_PumpEvents();
	SDL_GetMouseState(&xm, &ym);
	GLfloat lumpos[4] = {100, 30000, 100, 1.0}, cam[2];
	...
	if(_weaponMode==MODE_AVION)
	gl4duLookAtf(_cam.x, _cam.y /*1.0*/, _cam.z,_cam.x - sin(_cam.theta),_cam.y + (ym - (_windowHeight >> 1)) / (GLfloat)_windowHeight, _cam.z - cos(_cam.theta),0.0, 1.0,0.0);
	else
	gl4duLookAtf(_cam.x, _cam.y /*1.0*/, _cam.z,_cam.x - sin(_cam.theta),_cam.y - (ym - (_windowHeight >> 1)) / (GLfloat)_windowHeight, _cam.z - cos(_cam.theta),0.0, 1.0,0.0);
...
while(_ptc!=NULL){
    ...
    glDeleteVertexArrays(1, &_explosionVao);
    glDeleteBuffers(2, _explosionBuffer);
    glDeleteTextures(1, &_explosionTex);
